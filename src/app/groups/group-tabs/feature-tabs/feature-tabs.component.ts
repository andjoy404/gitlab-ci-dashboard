...
import { ConfigService, GlobalConfig } from '$service/config.service'
...
interface EnvironmentItem { id:number; namespace_id:number; name:string; base_url:string; group_ids:number[]; enabled:boolean; token_configured:boolean; is_default?:boolean }
const allTabs: Tab[] = [
...
const OPEN_ENVIRONMENT_SETUP_KEY='gcd_open_environment_setup'
...
export class FeatureTabsComponent implements OnInit {
  ...
  groupSelectorOpen=signal(false); environmentSelectorOpen=signal(false); groupSearch=signal(''); currentGroupId=computed(()=>[...this.groupMap().keys()][0]); currentNamespace=computed(()=>this.namespaceOf(this.currentGroupId())); currentEnvironment=computed(()=>this.environments().find(e=>e.namespace_id===this.currentNamespace())); environmentGroups=computed(()=>this.availableGroups().filter(g=>this.namespaceOf(g.id)===this.currentNamespace())); filteredGroups=computed(()=>{const q=this.groupSearch().trim().toLowerCase();const groups=this.environmentGroups();return q?groups.filter(g=>g.name.toLowerCase().includes(q)):groups}); environments=signal<EnvironmentItem[]>([])
  defaultEnvironmentId=signal<number|null>(null); selectedEnvironmentIds=signal<number[]>([]); editingEnvironmentId=signal<number|null>(null); highlightedEnvironmentId=signal<number|null>(null)
  ...
  setupOpen=signal(false); savingEnvironment=signal(false); setupError=signal(''); globalSaving=signal(false); globalMessage=signal(''); logoMode=signal<'url'|'upload'>('url'); logoFileName=signal(''); globalConfig: GlobalConfig={company_name:'GitLab CI Dashboard',company_logo:'',pipeline_view:'all'}
  ...
  ngOnInit(){ this.restoreEnvironmentHighlight();this.loadEnvironments(); if(this.auth.isAdmin())this.loadGlobalConfig(); this.openEnvironmentSetupIfRequested(); this.autoSelectDefaultEnvironment() }
  ...
  selectEnvironment(environment:EnvironmentItem){this.environmentSelectorOpen.set(false);this.defaultEnvironmentId.set(environment.id);const group=this.availableGroups().find(g=>this.namespaceOf(g.id)===environment.namespace_id);if(group){const feature=this.route.snapshot.paramMap.get('featureId')||'latest-pipelines';this.router.navigate([group.id,feature])}else{this.localTabIndex.set(3);this.router.navigate(['/'])}}
  selectGroup(group:Group){this.groupSelectorOpen.set(false);const feature=this.route.snapshot.paramMap.get('featureId')||'latest-pipelines';this.router.navigate([group.id,feature])}
  ...
  saveEnvironment(){const name=this.environment.name.trim(),base_url=this.environment.base_url.trim().replace(/\/$/,''),token=this.environment.token.trim();if(!name||!base_url||(!token&&!this.editingEnvironmentId())){this.setupError.set('Environment name, GitLab URL, and access token are required.');return}const group_ids=this.environment.group_ids.split(',').map(v=>Number(v.trim())).filter(v=>Number.isSafeInteger(v)&&v>0);const payload={name,base_url,token,group_ids,enabled:true,only_top_level:true,include_subgroups:true};const editingId=this.editingEnvironmentId();this.savingEnvironment.set(true);const req=editingId?this.http.patch(`api/environments/${editingId}`,payload):this.http.post('api/environments',payload);req.pipe(finalize(()=>this.savingEnvironment.set(false))).subscribe({next:(response:any)=>{const highlightId=editingId??Number(response?.id);this.setupOpen.set(false);if(Number.isSafeInteger(highlightId))this.highlightEnvironment(highlightId);this.loadEnvironments();this.environmentsChanged.emit()},error:(response)=>this.setupError.set(this.extractErrorMessage(response,'Unable to save environment.'))})}
  loadGlobalConfig(){this.http.get<GlobalConfig>('api/global-config').subscribe(v=>{this.globalConfig=v;this.config.setGlobalConfig(v)})}
  saveGlobalConfig(){const company_name=this.globalConfig.company_name.trim();if(!company_name){this.globalMessage.set('Company name is required.');return}const pipeline_view=this.globalConfig.pipeline_view.trim() as 'all' | 'latest';if(pipeline_view!=='all'&&pipeline_view!=='latest'){this.globalMessage.set('Invalid pipeline view selection.');return}this.globalSaving.set(true);const config: GlobalConfig={company_name,pipeline_view,company_logo:this.globalConfig.company_logo};this.http.put('api/global-config',config).pipe(finalize(()=>this.globalSaving.set(false))).subscribe({next:()=>{this.config.setGlobalConfig(config);this.globalMessage.set('Global configuration saved.');window.setTimeout(()=>{if(this.globalMessage()==='Global configuration saved.'){this.globalMessage.set('')}},3000)},error:({error})=>this.globalMessage.set(error?.message||'Unable to save global configuration.')})}
  onPipelineViewChange(value:'all'|'latest'){this.globalConfig.pipeline_view=value;this.config.setPipelineView(value)}
  loadEnvironments(){this.http.get<EnvironmentItem[]>('api/environments').subscribe(v=>{this.environments.set(v);const defaultEnv=v.find(e=>e.is_default);this.defaultEnvironmentId.set(defaultEnv?.id??null);this.autoSelectDefaultEnvironment()})}
  ...
  setDefaultEnvironment(id:number){this.http.patch(`api/environments/${id}/set-default`,{}).subscribe(()=>{this.loadEnvironments()})}
  ...
  private autoSelectDefaultEnvironment(){const defaultEnv=this.environments().find(e=>e.is_default);if(defaultEnv&&!this.emptyState()){const group=this.availableGroups().find(g=>this.namespaceOf(g.id)===defaultEnv.namespace_id);if(group){const currentGroupId=this.currentGroupId();if(currentGroupId!==group.id){const feature=this.route.snapshot.paramMap.get('featureId')||'latest-pipelines';window.setTimeout(()=>this.router.navigate([group.id,feature]),50)}}}}
  private openEnvironmentSetupIfRequested(){try{if(sessionStorage.getItem(OPEN_ENVIRONMENT_SETUP_KEY)==='true'){sessionStorage.removeItem(OPEN_ENVIRONMENT_SETUP_KEY);window.setTimeout(()=>this.openEnvironmentSetup(),0)}}catch{}}
  private extractErrorMessage(response: unknown, fallback: string): string {
